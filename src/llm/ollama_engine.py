...
class OllamaEngine:
    ...
    def __init__(
        ...
        Args:
            model_name: 模型名称（如 qwen2.5:7b, mistral, llama3 等）
            api_base: Ollama API 地址
            api_key: API 密钥（Ollama 通常不需要，设为任意值）
            temperature: 采样温度
        ...
        # 初始化 OpenAI 客户端（Ollama 兼容 OpenAI API）
        self.client = OpenAI(
            api_key=api_key,
            base_url=api_base,
        )

        logger.info(f"OllamaEngine initialized with model: {model_name}")
